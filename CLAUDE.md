...
**Last Updated**: 2026-07-23 (Hormozi pivot v2, gogo build)
**Current Status**: HORMOZI PIVOT v2 built end-to-end on `dev` per `handoff-pivot-build.md` + Russ's plan-file notes (notes override plan). The free front door is in: hero primary CTA is the free leak scan (Book a Call demoted to secondary), pick-your-path section + `/paths` (Launch/Growth/Signature, live exemplars incl. styx-stones-bones.pages.dev), `/automation` wedge (capability-tense, ZERO Upwork-profile numbers per Russ's 7/22 note), `/free-build` (5 slots, honest trade, 4-qualifier application; slot 1 = honey), `/case-studies` hub (numbers publish only with client sign-off; honey draft internal at `directives/case-study-honey-draft.md`), `/tools` hub + 7 tools (leak scanner + AI visibility = concierge same-day; slow-site, missed-lead, local grade, hire-vs-automate, automation-score = client-side calculators with visible adjustable assumptions; shared `ToolShell.astro`), AEO layer live (insights+guides collections, 10 pillar pages + 50 guides, strict pillar/cluster, Article+FAQPage JSON-LD, every guide feeds a tool; 49 guides via managed Opus fan-out, mechanical sweep 50/50 clean + independent review gate). Claims cleanup shipped: 96/100 -> dated 100/86 receipts linking /process, "Zero abandoned" killed, unsourced stats replaced by tool pointers, reviews source-linked (Jonny->Upwork profile, Carson->live site), testimonial wall = Jonny + Carson + Leon + Dirty Chai (dup killed). Verified-stat registry at `directives/tools-research.md` (folklore list included; "1s=7%" confirmed folklore). Qualifier dropdowns + Hormozi success-routing on all forms. Deploy gate script `execution/check-dist.mjs` (87 URLs clean). Analytics: CF Web Analytics needs Russ's 1-click Pages toggle (API tokens lack scope, verified); scoreboard seeded at `directives/scoreboard.md`. VSL slots marked in hero + /automation; 4 teleprompter scripts at `directives/vsl-scripts.md`. Jen review-ask Gmail draft r4153322469660183323 HELD. kit_gates: pivot 4-move batch + honey publish gate. Light-mode handoff still parked; `redesign/v2` still parked, never merge.
**New Files Added**:

- `src/pages/{automation,free-build,paths}.astro`, `src/pages/case-studies/index.astro`, `src/pages/tools/` (hub + 7), `src/components/ToolShell.astro`
- `src/pages/{insights,guides}/` layouts + `src/content/{insights,guides}/` (10 pillars + 50 guides), collections in `src/content.config.ts`
- `directives/{vsl-scripts,scoreboard,warm-50,case-study-honey-draft,blog-pipeline,tools-research,aeo-guide-brief}.md`, `execution/check-dist.mjs`
  **Prior state (2026-07-15)**: gap-pages build merged (`/compare`, `/event-websites`, `/process`, home FAQ); path-to-90 leftovers (LCP diet, per-page OG images, CRO pass) still ride with the parked light-mode pass; Russ's ~25-min SEO batch (DMARC rua, Bing import, authority hour) still open.
  **Old file list (2026-07-15)**:
