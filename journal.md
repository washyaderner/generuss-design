...
## 2026-03-26 11:30 | SESSION

**Context:** Moo rep recommended pulling card content 1-2mm inside safe area for cutting tolerance. Also needed cyan color unified between QR and card elements.
**Outcome:** Increased padding on both card sides (front 48->55px, back 38->55px), made surgical font adjustments to prevent overflow (back-brand 26->25px, back-url 15->14px), unified cyan from #00FFEF to #00E5FF across CSS + rgba + inline SVGs. Moved SHIPPED 2026 from both sides to back-only footnote. All 4 PDFs regenerated, QA-verified with magenta overlay proofs, mask polarity checks, and dimension validation.
**Signal:** Clean session - zero corrections, every change approved on first pass. "perfect" on SHIPPED 2026 repositioning.
**Friction:** Artwork PDFs weren't auto-generated by render pipeline (only masks get pngToPdf). Had to manually convert. Overlay proof compositing took 3 attempts to get sharp's API right.
**Carries forward:** 4 PDFs ready to upload to Moo for reprint.
...
## 2026-03-31 23:10 | SHIP | acec64e..6c380a5

Shipped: Conversion-first page restructure from Pharallax self-analysis
Commits: 24 since last ship
Key changes:

- Reordered homepage: Hero > Portfolio > Services > Reviews > P/S > About > Booking (was Hero > Booking > Services > Portfolio > P/S > Principles > About > Reviews)
- Dropped Principles section entirely (redundant with Problem/Solution)
- Removed 2 music production testimonials, kept 3 relevant web/AI reviews
- Shrunk Pharallax services card from massive 2-col report preview to compact card
- Updated P/S headline and About bio copy for ICP alignment
- Nav "Work" link points to portfolio instead of services
  Lesson: Booking should always be last. Asking for commitment before showing proof is the #1 conversion killer on service pages.

## 2026-04-08 | SHIP | 8836ca9..aa1c98a

Shipped: Responsive portfolio images + lazy-load optimization
Commits: 2 since last ship
Key changes:

- Generated 960w variants of all 11 desktop slides (856KB -> 284KB) with srcset/sizes
- Lazy-loaded all first portfolio slides (previously eager, all below fold)

## 2026-04-08 | SHIP | 6c380a5..8836ca9

Shipped: Hero copy line break + cross-browser animation fixes
Commits: 4 since last ship
Key changes:
